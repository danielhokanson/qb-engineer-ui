...
import { PartFlagsStepComponent } from './part-flags-step.component';
...
function buildPart(overrides: Partial<PartDetail> = {}): PartDetail {
  return {
    id: 42, partNumber: 'PRT-00042', name: 'Phantom A', description: null, revision: 'A',
    status: 'Draft',
...
describe('PartFlagsStepComponent (Phase 5 — save-on-Continue)', () => {
  let httpMock: HttpTestingController;
  let workflowService: WorkflowService;

  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [PartFlagsStepComponent],
      providers: [
        provideHttpClient(),
  ...
  it('renders without errors when entity is null', () => {
    const component = TestBed.runInInjectionContext(() => new PartFlagsStepComponent());
    mockSignalInputs(component, {
      stepId: 'flags', componentName: 'PartFlagsStepComponent',
  ...
  it('PATCHes /workflows/:runId/step when WorkflowService.saveCurrentStep() fires after a user edit', () => {
    ...
    form.patchValue({ isKit: true, backflushPolicy: 'Auto' });
    form.markAsDirty();
    ...
    expect(req.request.body.stepId).toBe('flags');
    expect(req.request.body.fields.isKit).toBe(true);
    expect(req.request.body.fields.backflushPolicy).toBe('Auto');
    req.flush({
      id: 7, entityType: 'Part', entityId: 42, definitionId: 'd', currentStepId: 'flags',
    ...
    partReq.flush(buildPart({ isKit: true, backflushPolicy: 'Auto' }));
